...
built "extensions" via Module Federation — and demonstrates, side by side,
...
you can compare them directly. See "The two approaches, in depth" below.
...
a task list) in their own Pinia store (`store.js`), and the host's sidebar
shows a live "Extension State" panel summarizing both — e.g. "Extension A: 3
...
component. This was verified with a headless-browser test.
...
5. **In-place hot-swap**: each manifest entry's `lastModifiedUnixMs` lets
   `useExtensionRegistry.js` tell "never seen" from "seen, but rebuilt
   since" from "unchanged." On a rebuild (e.g. `npm run dev:watch`'s
   `vite build --watch` writing straight into `wwwroot/apps/extensions/`),
   it re-imports the remote with a cache-busted URL (`?t=<mtime>` — the
   ...
   `router.replace()`. Found empirically: the underlying state updated
   correctly without it, but the page silently kept rendering the old
   component.
...
component remount (which is what a "swap" actually is — not a granular DOM
patch) is invisible in practice: verified by clicking the task list,
editing the source, and watching the edit appear with the checkbox state
intact.
...
earlier investigation (see "How this project got here" below) landed on:
...
entry at all), and the literal `import('extension-a/Extension')` call threw
...
current API — this project keeps one of each so you can see and compare
both rather than picking one and hiding the trade-off.
...
Swapping to `@module-federation/vite` (actively maintained, explicit Vite
8 support, a real `dev.remoteHmr` implementation) both unblocked the Vite
upgrade and made true live HMR possible — for the one case (declarative)
...
- `extension-b` in `vite build --watch` mode, writing straight into
  ...
  (rebuild + SSE + swap, not instant, but automatic) and the host swaps in
  the new component with its Pinia state untouched
...
5. With the backend already running, the `FileSystemWatcher` notices the new
   folder and pushes an SSE event — the sidebar gains "Extension C" without
   a page reload.
